...
#from ocrolib.hwocr import  img_prep
...
def eval():
    ...
    model = model_from_json(loaded_model_json)
    load_model_weights('./save/M7_1-all_weights.h5', model)
    X_train, y_train, X_test, y_test = pickle.load(open('./data/full.pkl', 'rb'))
        ...
        for name in files:
            if 'DAVID' not in name and 'line' not in name and 'checkbox' not in name :
                ...
                if filename[-4:]!='.txt' and \
                        ('.jpg' in filename or '.png' in filename or '.JPG' in filename or '.PNG' in filename):
                    str_re=predict_single(filename, model, label2chars)
...
def load_model_report2nissay_number_sep_kata_kanji(data_dir):
    ...
    khsave_m = data_dir + "/data/model_kanji_hira.json"
    khsave_w = data_dir + "/weight/weight_kanji_hira.h5"
    ...
    khsave_w = data_dir + "/weight/weights_m8_etl9.h5"
        ...
        for root, dirs, files in os.walk(root_dir):
            for name in files:
                ...
                if 'DAVID' not in name and 'line' not in name and 'checkbox' not in name:
                    ...
                    if filename[-4:] != '.txt' and \
                            ('.jpg' in filename or '.png' in filename or '.JPG' in filename or '.PNG' in filename):
                        str_re, prob = predict_single_sep_number_kata_kanji_old(filename,
                                                                      model,  label2chars,
                                                                      nmodel, nlabel2chars,
                                                                      kmodel, klabel2chars,
                                                                      khmodel, khlabel2chars,
...
def report2nissay_number_sep_kata_kanji_accuracy_test(root_dir, data_dir):
    ...
    model, nmodel, kmodel, khmodel, label2chars, nlabel2chars, klabel2chars, khlabel2chars = load_model_report2nissay_number_sep_kata_kanji(data_dir + "/model")
        ...
        for name in files:
            ...
            if 'line' not in name and 'checkbox' not in name and 'mixedline' not in name and 'labeled' in root.split(os.path.sep)[-1]:
                ...
                if filename[-4:] != '.txt' and \
                    ...
                    if true_label != 'X':
                        results.append({'filename':filename, 'template': root.split(os.path.sep)[-5], 'type':type_name[type], 'true_label':true_label, 'ocr_result':str_re, 'prob':prob})
    # export as csv
    #results = pickle.load(open('results.pkl','rb'))
    #print(results)
    pickle.dump(results, open('results.pkl','wb'))
    keys = results[0].keys()
    with open('accuracy_test.csv', 'wt') as output_file:
...
if __name__=='__main__':
    ...
    # report2('./test_images/cell_cut_for_OCR/')
    ...
    report2nissay_number_sep_kata_kanji_accuracy_test('/home/taprosoft/Downloads/test_segmented/nissay/run/test_label/temp3+4','/home/taprosoft/Downloads/test_segmented/ocropy/data')
    # make_gallery10(fpath='./data/full.pkl')

    # report_images('./test_images/Pros_Yamato_Data_Input.img/',)

    # X_train, y_train, X_test, y_test = pickle.load(open('./data/full_nissay.pkl', 'rb'))
    # mylabel2char = json.load(open('./data/full_nissay.label2char.json'))
    # for i in range(X_train.shape[0]):
    #     print(mylabel2char[str(y_train[-i-1])])
    #     plt.imshow(X_train[-i-1][0])
    #     plt.show()

    # load_data_npz()
    # train('./data/full_katakana.pkl', "./save/model_katakana.json", "./save/weight_katakana.h5")
    #train('./data/full_katakana_quote.pkl', "./save/model_katakana_quote.json", "./save/weight_katakana_quote.h5")
