...
def do_cropping(image, negated=False, max_cobj=3, ignore_width=False, ignore_height=False):
    """
    ...
    try:
        max_index = numpy.argsort(numpy.asarray([len(c) for c in contours]))
    except ValueError:
        return image

    # find the cropping co-ordinates
    x, y, width, height = cv2.boundingRect(contours[max_index[-1]])

    for k in range(max_cobj-1):
        if len(max_index)>k+1:
            x1=x
            y1=y
            width1=width
            height1=height
            x2, y2, width2, height2 = cv2.boundingRect(contours[max_index[-k-2]])

            if (x<=x2+width2<=x+width or x<=x2<=x+width) or (y<=y2+height2<=y+height or y<=y2<=y+height)\
              or (x2<=x+width<=x2+width2 or x2<=x<=x2+width2) or (y2<=y+height<=y2+height2 or y2<=y<=y2+height2):
                x=min(x,x2)
                y=min(y,y2)
    ...
    if ignore_height and not ignore_width:
        cropped_img = image[:, x:x + width]
    elif ignore_width and not ignore_height:
        cropped_img = image[y:y + height,:]
    elif not ignore_width and not ignore_height:
...
def elastic_transform(image, kernel_dim=13, sigma=6, alpha=36, negated=False):
    ...
    displacement_field_x = numpy.array([[random_integers(-1, 1) for x in range(image.shape[0])] \
                                        for y in range(image.shape[1])]) * alpha
    displacement_field_y = numpy.array([[random_integers(-1, 1) for x in range(image.shape[0])] \
                                        for y in range(image.shape[1])]) * alpha
