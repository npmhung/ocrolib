...
def data(writers_per_char=160, mode='all', get_scripts=False, test_size=0.2):
    ...
    if mode in ('kanji', 'all'):
        for i in range(1, 4):
            ...
            if i == 1 and mode in ('kanji', 'all'):
                ...
                jiscodes = np.concatenate((jiscodes, codes), axis=0)
                if get_scripts:
    ...
    if mode in ('katakana', 'all'):
        for i in range(7, 14):
            ...
            if get_scripts:
                chars, labs, spts, codes = get_ETL_data(filename, range(
                    0, 8), writers_per_char, database='ETL1C', get_scripts=True)
            else:
                chars, labs, codes = get_ETL_data(filename, range(
    ...
    return X_train, Y_train, X_test, Y_test, label2codes
