################################################################
### various add-ons to the SciPy morphology wordcut
################################################################
...
def center_mass(input, labels=None, index=None):
    return measurements.center_of_mass(input, labels, index)
...
def fill_hole(image):
    """binary fill hole"""
    #struct_el = morphology.generate_binary_structure(size, 4)
    return morphology.binary_fill_holes(image).astype(int)
