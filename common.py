...
def write_text(fname,text,nonl=0,normalize=1):
    ...
    with codecs.open(fname,"w","utf-8") as stream:
        stream.write(text)
        if len(text) > 0 and not nonl and text[-1]!='\n':
            stream.write('\n')
...
#@checks(str,_=ABINARY2)
def read_image_binary_small(fname,dtype='i',pageno=0, width = 16000):
    """Read an image from disk and return it as a binary image
    of the given dtype."""
    if type(fname)==tuple: fname,pageno = fname
    assert pageno==0
    pil = PIL.Image.open(fname)
    w = width
    h = w * pil.size[0] / pil.size[1]
    a = pil2array(pil)
    print("Resizing...")
    pil.thumbnail((w, h), PIL.Image.BICUBIC)
    a_small = pil2array(pil)
    print("Done")
    if a.ndim==3: a = amax(a,axis=2)
    if a_small.ndim == 3: a_small = amax(a_small, axis=2)
    return a, array(a_small>0.1*(amin(a_small)+amax(a_small)),dtype)
...
def pad_by(image,r,dtype=None,invert=True):
    """Symmetrically pad the image by the given amount.
    FIXME: replace by scipy version."""
    if dtype is None: dtype = image.dtype
    w,h = image.shape
    if invert:
        result = zeros((w+2*r,h+2*r)).astype(dtype)
    else:
        result = ones((w + 2 * r, h + 2 * r)).astype(dtype)
