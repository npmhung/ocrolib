...
def check_data(data_dir, num=100, store_path='./test_images/check/'):
    ...
    for ci,i in enumerate(indexs):
        ...
        # print(np.max(im))
        # print(np.min(im))
        cv2.imwrite(store+'/{}.png'.format(ci),im*255)
        if ci>num:
            break
    ...
    for ci, i in enumerate(indexs):
        im = X_test[i][0]
        cv2.imwrite(store + '/{}.png'.format(num+ci), im*255)
        if ci > num:
            break
...
def report_extract_excel(root_dir, label2chard = './data/mnist_full.nice.noise.label2char.json',
                  save_m ="./save/model_mnist_full.nice.noise.json",
                         ...
                         out_wrong='./test_images/excel_wrong/'):
    print(root_dir)
    from keras.models import model_from_json
    print('start load...')
    label2chars = json.load(open(label2chard))
...
if __name__=='__main__':
    ...
    # remake_data_noise('./data/mnist_full.nice.pkl')
    # check_data('./data/full_katakana.pkl')
    # split_kanji_hira()
    ...
    #                      save_m="./save/model_katakana_quote.nice.json", save_w='./save/weight_katakana_quote.nice.h5')
