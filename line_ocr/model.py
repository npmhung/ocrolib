...
layer_params = [
                 ...
                 [  64, 3, 'valid', 'conv0', False],
                [  64, 3, 'same',  'conv02', True],] # pool
...
def convnet_layers(inputs, widths, mode):
    ...
    with tf.variable_scope("convnet"): # h,w
        ...
        pool8 = tf.reshape(tf.transpose(conv8, [0,2,1,3]), [32, -1, 512*3])
        print(pool8.shape)
...
def rnn_layer(bottom_sequence,sequence_length,rnn_size,scope,nlayer=1):
    ...
    if nlayer==1:
        ...
        cell_fw = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(rnn_size, initializer=weight_initializer) for _ in range(nlayer)])
        cell_bw = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(rnn_size, initializer=weight_initializer) for _ in range(nlayer)])
    # Include?
...
def rnn_layers(features, sequence_length, num_classes, nlayer=1):
    ...
    with tf.variable_scope("rnn"):
        ...
        rnn1 = rnn_layer(rnn_sequence, sequence_length, rnn_size, 'bdrnn1', nlayer)
        rnn2 = rnn_layer(rnn1, sequence_length, rnn_size, 'bdrnn2', nlayer)
...
def ctc_loss_layer(rnn_logits, sequence_labels, sequence_length):
    ...
    #loss = tf.nn.ctc_loss( sequence_labels, rnn_logits, sequence_length,
                           #time_major=True )
    loss = tf.nn.ctc_loss(sequence_labels, rnn_logits, sequence_length,
						ctc_merge_repeated=False, preprocess_collapse_repeated=False,
